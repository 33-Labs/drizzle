...
export const toggleClaimable = async (
  dropID,
  setTransactionInProgress,
  setTransactionStatus
) => {
  const txFunc = async () => {
    return await doToggleClaimable(dropID)
  }
...
const doToggleClaimable = async (dropID) => {
  const code = `
  import DropN from 0xDrop
