...
const MemoizeBasicInfo = React.memo(({host, createdAt, timeLockEnabled, startAt, endAt}) => {
  return (
...
export default function DropCard(props) {
  ...
  // Only created Drop has status
  const status = props.status
  const token = props.token
  const amount = (status && status.amount) || props.amount
  ...
  return (
    <div className="flex flex-col w-[480px] min-w-[320px] shadow-[0px_5px_25px_-5px_rgba(0,0,0,0.1)] mt-5 mb-10 items-stretch">
      ...
      <MemoizeBasicInfo 
        ...
        timeLockEnabled={props.timeLockEnabled} startAt={props.startAt} endAt={props.endAt}
      />
